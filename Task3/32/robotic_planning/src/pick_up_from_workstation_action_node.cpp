...
public:
  ...
  : plansys2::ActionExecutorClient("pick-up-from-workstation", 250ms)
  {
    progress_ = 0.0;
  }
...
private:
  ...
  { std :: vector <std :: string > arguments = get_arguments () ;
    if (progress_ < 1.0) {
      ...
      finish(true, progress_, "agent "+arguments [0]+ " is picking up "+ 
            arguments [1]+ " in "+ arguments [2] + " carring a "+
            arguments [3]);
...
int main(int argc, char ** argv)
{
  ...
  node->set_parameter(rclcpp::Parameter("action_name", "pick-up-from-workstation"));
